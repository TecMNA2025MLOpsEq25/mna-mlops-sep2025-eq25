...
def plot_roc_curves_one_model(name, y_true_b, probas, classes, outdir: Path):
    ...
    # Devolvemos también la curva macro
    return roc_auc_macro, all_fpr, mean_tpr
...
def plot_pr_curves_one_model(name, y_true_b, probas, classes, outdir: Path):
    ...
    # Plot macro/micro (solo micro en la figura principal)
    plt.figure(figsize=(7, 6))
...
def main():
    ...
    # 2) Curvas por modelo
    macro_roc_all = {}  # name -> {"auc": float, "fpr": array, "tpr": array}
    macro_pr_all = {}   # name -> ap_macro
    ...
    for p in model_paths:
        ...
        if probas.shape[1] != len(classes):
            raise ValueError(
                f"{name}: salida proba tiene {probas.shape[1]} clases y se esperaban {len(classes)}."
            )

        roc_macro, fpr_macro, tpr_macro = plot_roc_curves_one_model(
            name, y_test_b, probas, classes, outdir
        )
        pr_macro = plot_pr_curves_one_model(
            name, y_test_b, probas, classes, outdir
        )
        ...
        macro_pr_all[name] = float(pr_macro)
    ...
    # ROC macro compare: curvas reales
    plt.figure(figsize=(7, 6))
    for name, info in sorted(macro_roc_all.items(), key=lambda x: -x[1]["auc"]):
        plt.plot(
            info["fpr"],
            info["tpr"],
            lw=2,
    ...
    # PR macro compare (barras con AP macro)
    plt.figure(figsize=(7, 6))

    # ordenamos de mayor a menor AP
    items = sorted(macro_pr_all.items(), key=lambda x: -x[1])
    names = [name for name, _ in items]
    vals  = [val for _, val in items]

    plt.bar(names, vals)
    plt.ylabel("AP macro")
    plt.ylim(0.9, 1.0)  # ajusta el rango si quieres más/menos zoom
    ...
    pd.DataFrame(roc_summary).sort_values(
        "roc_auc_macro", ascending=False
    ).to_csv(OUT_ROC, index=False)
    pd.DataFrame(pr_summary).sort_values(
        "ap_macro", ascending=False
    ).to_csv(OUT_PR, index=False)
